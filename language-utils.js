 ...
 */
function addLocalizedImage(screenshotIndex, lang, image, src, name, extra = {}) {
...
function handleTranslationFileSelect(event) {
    ...
    if (file.type.startsWith('image/')) {
        ...
        reader.onload = (e) => {
            const img = new Image();
            img.onload = () => {
                addLocalizedImage(currentTranslationsIndex, lang, img, e.target.result, file.name, { type: 'image' });
                updateScreenshotTranslationsList();
            };
    ...
    // Video (store blob in IndexedDB, extract frame as poster image)
    if (file.type.startsWith('video/')) {
        (async () => {
            if (typeof putMediaBlob !== 'function' || typeof extractVideoFrameFromBlob !== 'function') {
                return;
            }
            ...
            updateScreenshotTranslationsList();
        })().catch(err => console.error('Error adding localized video:', err));
