...
const state = {
    ...
    selectedMediaIndices: new Set(), // Indices of media items selected for bulk remove
    ...
    customWidth: 1290,
    customHeight: 2796,
    exportTransparentBackground: false, // Export only framed media with transparency
...
function isLegacyDefaultGradientBackground(bg) {
    if (!bg || bg.type !== 'gradient') return false;
    if (!bg.gradient || bg.gradient.angle !== 135) return false;
    const stops = bg.gradient.stops;
    if (!Array.isArray(stops) || stops.length !== 2) return false;

    const s0 = stops[0];
    const s1 = stops[1];
    const c0 = (s0?.color || '').toLowerCase();
    const c1 = (s1?.color || '').toLowerCase();
    const p0 = s0?.position;
    const p1 = s1?.position;

    if (c0 !== '#667eea' || c1 !== '#764ba2') return false;
    if (p0 !== 0 || p1 !== 100) return false;

    const solid = (bg.solid || '').toLowerCase();
    if (solid !== '#1a1a2e') return false;

    return true;
}

function applyNewDefaultSolidBackground(bg) {
    if (!bg) return;
    bg.type = 'solid';
    bg.solid = '#F8F9FA';
}
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            request.onupgradeneeded = (event) => {
                ...
                // Store for large binary media (e.g. uploaded videos)
...
function updateProjectSelector() {
    ...
    document.getElementById('project-trigger-meta').textContent = `${count} media item${count !== 1 ? 's' : ''}`;
    ...
    projects.forEach(project => {
        ...
        option.innerHTML = `
            ...
            <span class="project-option-meta">${screenshotCount} media item${screenshotCount !== 1 ? 's' : ''}</span>
        `;
...
function putMediaBlob(blob, meta = {}) {
...
function getMediaBlob(key) {
    if (!db || !key) return Promise.resolve(null);
    return new Promise((resolve) => {
        try {
...
function waitForCondition(conditionFn, timeoutMs = 20000, pollMs = 50) {
    return new Promise((resolve, reject) => {
        ...
        const tick = () => {
            try {
                ...
                // ignore condition errors and keep polling
            }
...
async function extractVideoFrameFromBlob(blob, timeSeconds = 0) {
    ...
    try {
        ...
        const safeMax = duration > 0 ? Math.max(0, duration - 0.05) : 0;
        const safeTime = Math.max(0, Math.min(timeSeconds || 0, safeMax));

        // Some browsers won't fire seeked for 0 -> 0; nudge slightly if possible.
        const targetTime = safeTime === 0 && safeMax > 0 ? Math.min(0.05, safeMax) : safeTime;
        try {
            video.currentTime = targetTime;
            await waitForEvent(video, 'seeked', 15000);
        } catch (e) {
            // Fallback: loadeddata is often enough for first frame
            try { await waitForEvent(video, 'loadeddata', 15000); } catch (_) {}
        }
            ...
            Object.keys(s.localizedImages).forEach(lang => {
                ...
                if (langData?.src) {
                    localizedImages[lang] = {
                        src: langData.src,
                        name: langData.name,
                        type: langData.type || 'image',
                        videoKey: langData.videoKey || null,
                        videoName: langData.videoName || null,
                        videoFrameTime: Number.isFinite(langData.videoFrameTime) ? langData.videoFrameTime : null,
                        videoDuration: Number.isFinite(langData.videoDuration) ? langData.videoDuration : null,
                        videoWidth: Number.isFinite(langData.videoWidth) ? langData.videoWidth : null,
                        videoHeight: Number.isFinite(langData.videoHeight) ? langData.videoHeight : null
                    };
                }
            });
        }
        ...
        try {
            ...
            request.onsuccess = () => {
                ...
                if (parsed) {
                                    ...
                                    if (langData?.src) {
                                        ...
                                        langImg.onload = () => {
                                            localizedImages[lang] = {
                                                ...
                                                videoKey: langData.videoKey || null,
                                                videoName: langData.videoName || null,
                                                videoFrameTime: Number.isFinite(langData.videoFrameTime) ? langData.videoFrameTime : 0,
                                                videoDuration: Number.isFinite(langData.videoDuration) ? langData.videoDuration : null,
                                                videoWidth: Number.isFinite(langData.videoWidth) ? langData.videoWidth : null,
                                                videoHeight: Number.isFinite(langData.videoHeight) ? langData.videoHeight : null
                                            };
                    ...
                    state.exportTransparentBackground = !!parsed.exportTransparentBackground;
                    ...
                    // If this project is still using the old "default purple gradient",
                    // switch defaults (and any unchanged screenshot backgrounds) to solid #F8F9FA.
                    if (isLegacyDefaultGradientBackground(state.defaults?.background)) {
                        applyNewDefaultSolidBackground(state.defaults.background);

                        // Also update existing screenshots that still have the untouched legacy default background
                        state.screenshots.forEach(s => {
                            if (isLegacyDefaultGradientBackground(s?.background)) {
                                applyNewDefaultSolidBackground(s.background);
                            }
                        });
                    }
                } else {
...
function syncUIWithState() {
    ...
    // Export options
    const exportTransparentToggle = document.getElementById('export-transparent-toggle');
    if (exportTransparentToggle) {
        exportTransparentToggle.checked = !!state.exportTransparentBackground;
    }
    ...
    // Video frame controls (only for video screenshots in current language)
    syncVideoFrameControlsUI();
...
function setupEventListeners() {
    ...
    // Export transparent toggle (export only framed media)
    const exportTransparentToggle = document.getElementById('export-transparent-toggle');
    if (exportTransparentToggle) {
        exportTransparentToggle.addEventListener('change', (e) => {
            state.exportTransparentBackground = !!e.target.checked;
            saveState();
        });
    }
    ...
    // Media bar: select all / remove selected
    document.getElementById('media-select-all-checkbox').addEventListener('change', (e) => {
        if (e.target.checked) {
            state.screenshots.forEach((_, i) => state.selectedMediaIndices.add(i));
        } else {
            state.selectedMediaIndices.clear();
        }
        updateScreenshotList();
    });
    document.getElementById('media-remove-selected').addEventListener('click', () => {
        removeSelectedMedia();
    });
    ...
    document.getElementById('developer-info-modal').addEventListener('click', (e) => {
        if (e.target.id === 'developer-info-modal') {
            document.getElementById('developer-info-modal').classList.remove('visible');
        }
    });
    ...
    // Video frame picker (visible only when current screenshot is a video)
    const videoFrameSlider = document.getElementById('video-frame-time');
    const videoFrameValue = document.getElementById('video-frame-time-value');
    if (videoFrameSlider && videoFrameValue) {
        videoFrameSlider.addEventListener('input', (e) => {
            const t = parseFloat(e.target.value);
            videoFrameValue.textContent = `${Number.isFinite(t) ? t.toFixed(1) : '0.0'}s`;

            // Seek live preview immediately (so it "renders as video" while scrubbing)
            const asset = getCurrentLocalizedAsset();
            if (asset?.type === 'video' && _videoPreviewState.videoEl && asset.videoKey === _videoPreviewState.videoKey) {
                try { _videoPreviewState.videoEl.currentTime = Math.max(0, t); } catch (err) {}
            }
            // Seek Three.js video texture too (3D mode)
            if (asset?.type === 'video' && typeof setScreenVideoTime === 'function') {
                try { setScreenVideoTime(Math.max(0, t)); } catch (err) {}
            }

            if (_videoFrameDebounceTimer) clearTimeout(_videoFrameDebounceTimer);
            _videoFrameDebounceTimer = setTimeout(() => {
                updateVideoFrameForCurrentScreenshot(t).catch(err => console.error('Video frame update failed:', err));
            }, 200);
        });
    }
...
Translate to these language codes: ${targetLangs.join(', ')}`;
        ...
        updateStatus('Applying translations...', 'Updating media');
...
function handleFiles(files) {
    ...
    processFilesSequentially(Array.from(files).filter(f => f.type.startsWith('image/') || f.type.startsWith('video/')));
}
...
async function processElectronFilesSequentially(filesData) {
    for (const fileData of filesData) {
        await processElectronMediaFile(fileData);
    }
}
...
async function processElectronMediaFile(fileData) {
    const mime = getMimeFromDataUrl(fileData?.dataUrl);
    if (mime.startsWith('video/')) {
        return processElectronVideoFile(fileData);
    }
    return processElectronImageFile(fileData);
}
...
async function processElectronVideoFile(fileData) {
    try {
        const blob = dataUrlToBlob(fileData.dataUrl);
        if (!blob) return;

        const detectedLang = detectLanguageFromFilename(fileData.name);
        const videoKey = await putMediaBlob(blob, { name: fileData.name, type: blob.type });

        const frame = await extractVideoFrameFromBlob(blob, 0);
        const posterSrc = frame.dataUrl;
        const posterImg = await new Promise((resolve, reject) => {
            const img = new Image();
        ...
        const extra = {
            ...
            videoFrameTime: frame.usedTime || 0,
        ...
        if (existingIndex !== -1) {
            ...
            if (hasExistingLangImage) {
                ...
                if (choice === 'replace') {
                    addLocalizedImage(existingIndex, detectedLang, posterImg, posterSrc, fileData.name, extra);
                } else if (choice === 'create') {
                    createNewScreenshot(posterImg, posterSrc, fileData.name, detectedLang, deviceType, extra);
                }
            } else {
                addLocalizedImage(existingIndex, detectedLang, posterImg, posterSrc, fileData.name, extra);
            }
        ...
        console.error('Error processing Electron video file:', e);
    }
}
...
async function processVideoFile(file) {
    try {
        ...
        // Store video blob (for later frame picking)
        const videoKey = await putMediaBlob(file, { name: file.name, type: file.type });

        // Extract initial frame
        const frame = await extractVideoFrameFromBlob(file, 0);
        const posterSrc = frame.dataUrl;
        ...
        if (existingIndex !== -1) {
            ...
            if (hasExistingLangImage) {
                ...
                if (choice === 'replace') {
                    addLocalizedImage(existingIndex, detectedLang, posterImg, posterSrc, file.name, extra);
                } else if (choice === 'create') {
                    createNewScreenshot(posterImg, posterSrc, file.name, detectedLang, deviceType, extra);
                }
            } else {
                addLocalizedImage(existingIndex, detectedLang, posterImg, posterSrc, file.name, extra);
            }
...
function createNewScreenshot(img, src, name, lang, deviceType, localizedExtra = {}) {
    const localizedImages = {};
    localizedImages[lang] = {
        image: img,
        src: src,
...
function updateScreenshotList() {
    ...
    if (state.transferTarget !== null && state.screenshots.length > 1) {
        ...
        hint.innerHTML = `
            <span>Select a media item to copy style from</span>
    ...
    state.screenshots.forEach((screenshot, index) => {
        ...
        // Checkbox for bulk selection (only when not in transfer mode)
        const checkboxHtml = !isTransferMode
            ? `<label class="media-item-checkbox" title="Select for bulk remove"><input type="checkbox" class="media-item-checkbox-input" data-index="${index}" ${state.selectedMediaIndices.has(index) ? 'checked' : ''}></label>`
            : '';
        ...
        item.addEventListener('click', (e) => {
            if (e.target.closest('.screenshot-menu-wrapper') || e.target.closest('.drag-handle') || e.target.closest('.media-item-checkbox')) {
                return;
            }
        ...
        const itemCheckbox = item.querySelector('.media-item-checkbox-input');
        if (itemCheckbox) {
            itemCheckbox.addEventListener('click', (e) => e.stopPropagation());
            itemCheckbox.addEventListener('change', () => {
                if (itemCheckbox.checked) {
                    state.selectedMediaIndices.add(index);
                } else {
                    state.selectedMediaIndices.delete(index);
                }
    ...
    if (state.transferTarget === null) {
        ...
        uploadItem.innerHTML = `
            ...
            <div class="screenshot-info">
                <div class="screenshot-name">Add Media</div>
...
function updateMediaSelectionUI() {
    const selectAllCheckbox = document.getElementById('media-select-all-checkbox');
    const removeBtn = document.getElementById('media-remove-selected');
    if (!selectAllCheckbox || !removeBtn) return;
    const n = state.screenshots.length;
    const selectedCount = state.selectedMediaIndices.size;
    selectAllCheckbox.checked = n > 0 && selectedCount === n;
    selectAllCheckbox.indeterminate = selectedCount > 0 && selectedCount < n;
    selectAllCheckbox.disabled = n === 0;
    removeBtn.disabled = selectedCount === 0;
}

function removeSelectedMedia() {
    if (state.selectedMediaIndices.size === 0) return;
    const indices = Array.from(state.selectedMediaIndices).sort((a, b) => b - a);
    const removedBeforeSelected = indices.filter(i => i < state.selectedIndex).length;
    indices.forEach(i => state.screenshots.splice(i, 1));
    state.selectedMediaIndices.clear();
    if (state.screenshots.length === 0) {
        state.selectedIndex = 0;
    } else {
        state.selectedIndex = Math.max(0, Math.min(state.screenshots.length - 1, state.selectedIndex - removedBeforeSelected));
    }
    updateScreenshotList();
    syncUIWithState();
    updateGradientStopsUI();
    ...
    fileInput.addEventListener('change', (e) => {
        ...
        if (file.type.startsWith('video/')) {
            (async () => {
                ...
                if (!screenshot.localizedImages) screenshot.localizedImages = {};
                ...
                saveState();
                syncUIWithState();
            })().catch(err => console.error('Error replacing with video:', err));
...
function updateCanvas() {
    ...
    if (state.screenshots.length > 0) {
        const ss = getScreenshotSettings();
        const use3D = ss.use3D || false;
        // Ensure Three.js is initialized whenever 3D is enabled
        if (use3D && typeof showThreeJS === 'function') {
            showThreeJS(true);
        }
    ...
    // If current screenshot is a video (2D), start/keep animated preview
    syncVideoPreviewPlayback().catch(() => null);
}

let _videoFrameDebounceTimer = null;
let _videoMetadataLoading = false;
let _videoPreviewToggleHandlerAttached = false;

const _videoPreviewState = {
    videoKey: null,
    url: null,
    videoEl: null,
    rafId: null,
...
function getCurrentLocalizedAsset() {
    const screenshot = getCurrentScreenshot();
    if (!screenshot?.localizedImages) return null;
    return screenshot.localizedImages[state.currentLanguage] || null;
}

function isExportTransparentEnabled() {
    return !!state.exportTransparentBackground;
}

function renderCurrentStateToPngDataUrl({ transparentOnlyFrame = false } = {}) {
    const dims = getCanvasDimensions();
    ...
    exportCtx.clearRect(0, 0, dims.width, dims.height);
    ...
    if (!transparentOnlyFrame) {
        drawBackgroundToContext(exportCtx, dims, bg);
        if (bg.noise) {
            drawNoiseToContext(exportCtx, dims, bg.noiseIntensity);
        }
    }
    ...
    if (use3D && typeof renderThreeJSToCanvas === 'function' && phoneModelLoaded) {
        ...
        renderThreeJSToCanvas(exportCanvas, dims.width, dims.height);
    } else if (!use3D) {
        const img = getScreenshotImage(getCurrentScreenshot());
        drawScreenshotToContext(exportCtx, dims, img, ss);
    }

    if (!transparentOnlyFrame) {
        drawTextToContext(exportCtx, dims, txt);
    }
...
function cleanupVideoPreview() {
    if (_videoPreviewState.rafId) {
        cancelAnimationFrame(_videoPreviewState.rafId);
        _videoPreviewState.rafId = null;
    }
    if (_videoPreviewState.raf3DId) {
        cancelAnimationFrame(_videoPreviewState.raf3DId);
        _videoPreviewState.raf3DId = null;
    }
    if (_videoPreviewState.videoEl) {
        try { _videoPreviewState.videoEl.pause(); } catch (e) {}
        try { _videoPreviewState.videoEl.removeAttribute('src'); _videoPreviewState.videoEl.load(); } catch (e) {}
    }
    if (_videoPreviewState.url) {
        try { URL.revokeObjectURL(_videoPreviewState.url); } catch (e) {}
    }
    _videoPreviewState.videoKey = null;
    _videoPreviewState.url = null;
    _videoPreviewState.videoEl = null;
}

async function ensurePreviewVideoElementForCurrentAsset() {
    const asset = getCurrentLocalizedAsset();
    const ss = getScreenshotSettings();
    const use3D = ss.use3D || false;
    if (!asset || asset.type !== 'video' || !asset.videoKey) return null;
    if (use3D) return null; // keep 3D as poster for now

    if (_videoPreviewState.videoEl && _videoPreviewState.videoKey === asset.videoKey) {
        return _videoPreviewState.videoEl;
    }

    cleanupVideoPreview();

    const blob = await getMediaBlob(asset.videoKey);
    if (!blob) return null;
    ...
    await waitForEvent(video, 'loadedmetadata', 15000);
    const startT = Number.isFinite(asset.videoFrameTime) ? asset.videoFrameTime : 0;
    try {
        video.currentTime = Math.max(0, startT);
        await waitForEvent(video, 'seeked', 15000);
    } catch (e) {
        // ignore
    }

    _videoPreviewState.videoKey = asset.videoKey;
    _videoPreviewState.url = url;
    _videoPreviewState.videoEl = video;
    return video;
}

function renderMainPreviewFrame({ preferVideo = false } = {}) {
    const dims = getCanvasDimensions();
    ...
    if (state.screenshots.length > 0) {
        ...
        if (use3D && typeof renderThreeJSToCanvas === 'function' && phoneModelLoaded) {
            ...
            if (preferVideo) {
                const videoEl = _videoPreviewState.videoEl;
                const asset = getCurrentLocalizedAsset();
                if (asset?.type === 'video' && videoEl) {
                    drawScreenshotToContext(ctx, dims, videoEl, ss);
                } else {
                    drawScreenshot();
                }
            } else {
...
async function syncVideoPreviewPlayback() {
    // Attach toggle handler once
    if (!_videoPreviewToggleHandlerAttached) {
        const btn = document.getElementById('video-preview-toggle');
        if (btn) {
            btn.addEventListener('click', async () => {
                _videoPreviewState.enabled = !_videoPreviewState.enabled;
                btn.textContent = _videoPreviewState.enabled ? 'Pause Video' : 'Play Video';
                await syncVideoPreviewPlayback();
            });
            _videoPreviewToggleHandlerAttached = true;
        }
    }
    ...
    // No video / not eligible
    if (!asset || asset.type !== 'video' || !asset.videoKey) {
        cleanupVideoPreview();
        const btn = document.getElementById('video-preview-toggle');
    ...
    // 3D mode: video should play inside the phone frame (Three.js texture)
    if (use3D) {
        // Ensure Three.js is initialized in 3D mode
        if (typeof showThreeJS === 'function') {
            showThreeJS(true);
        }

        // Stop 2D preview loop/video element
        if (_videoPreviewState.rafId) {
            cancelAnimationFrame(_videoPreviewState.rafId);
            _videoPreviewState.rafId = null;
        }
        ...
        // Tell Three.js video whether to play/pause (if available)
        if (typeof setScreenVideoPlaybackEnabled === 'function') {
            try { setScreenVideoPlaybackEnabled(_videoPreviewState.enabled); } catch (e) {}
        }

        const btn = document.getElementById('video-preview-toggle');
        if (btn) btn.textContent = _videoPreviewState.enabled ? 'Pause Video' : 'Play Video';

        if (_videoPreviewState.raf3DId) return;

        // Ensure Three.js screen texture is initialized (video)
        if (typeof updateScreenTexture === 'function') {
            updateScreenTexture();
        }

        // Composite Three.js output onto the main canvas each frame
        const loop3D = () => {
            const currentAsset = getCurrentLocalizedAsset();
            const currentSS = getScreenshotSettings();
            if (!currentAsset || currentAsset.type !== 'video' || !currentSS.use3D) {
                cleanupVideoPreview();
                return;
            }
            ...
            // 3D phone (with video texture handled inside three-renderer.js)
            const dims = getCanvasDimensions();
            if (typeof renderThreeJSToCanvas === 'function' && phoneModelLoaded) {
                renderThreeJSToCanvas(canvas, dims.width, dims.height);
            }
            ...
            _videoPreviewState.raf3DId = requestAnimationFrame(loop3D);
        };
        _videoPreviewState.raf3DId = requestAnimationFrame(loop3D);
        return;
    }

    // 2D mode: video animates by drawing the <video> into the canvas
    await ensurePreviewVideoElementForCurrentAsset();
    const video = _videoPreviewState.videoEl;
    if (!video) return;

    const btn = document.getElementById('video-preview-toggle');
    if (btn) btn.textContent = _videoPreviewState.enabled ? 'Pause Video' : 'Play Video';

    if (_videoPreviewState.enabled) {
        try { await video.play(); } catch (e) { /* autoplay might fail; still draw current frame */ }
        if (!_videoPreviewState.rafId) {
            const loop = () => {
                const currentAsset = getCurrentLocalizedAsset();
                const currentSS = getScreenshotSettings();
                if (!currentAsset || currentAsset.type !== 'video' || currentAsset.videoKey !== _videoPreviewState.videoKey || (currentSS.use3D || false)) {
                    cleanupVideoPreview();
                    return;
                }
                renderMainPreviewFrame({ preferVideo: true });
                _videoPreviewState.rafId = requestAnimationFrame(loop);
            };
            _videoPreviewState.rafId = requestAnimationFrame(loop);
        }
    } else {
        ...
        // Draw one static frame (poster) when paused
        renderMainPreviewFrame({ preferVideo: false });
    }
}

async function ensureCurrentVideoMetadata() {
    if (_videoMetadataLoading) return;
    const asset = getCurrentLocalizedAsset();
    if (!asset || asset.type !== 'video') return;
    if (Number.isFinite(asset.videoDuration) && asset.videoDuration > 0) return;
    if (!asset.videoKey) return;

    _videoMetadataLoading = true;
    try {
        const blob = await getMediaBlob(asset.videoKey);
        if (!blob) return;
        const meta = await extractVideoFrameFromBlob(blob, asset.videoFrameTime || 0);
        asset.videoDuration = meta.duration;
        asset.videoWidth = meta.width;
        asset.videoHeight = meta.height;
        saveState();
    } catch (e) {
        // ignore
    } finally {
        _videoMetadataLoading = false;
    }
}

function syncVideoFrameControlsUI() {
    const container = document.getElementById('video-frame-controls');
    const slider = document.getElementById('video-frame-time');
    const valueEl = document.getElementById('video-frame-time-value');
    if (!container || !slider || !valueEl) return;

    const asset = getCurrentLocalizedAsset();
    const isVideo = asset?.type === 'video';
    container.style.display = isVideo ? 'block' : 'none';
    if (!isVideo) return;

    const duration = Number.isFinite(asset.videoDuration) ? asset.videoDuration : 0;
    slider.max = duration > 0 ? duration.toFixed(3) : '0';
    slider.disabled = !(duration > 0 && !!asset.videoKey);

    const t = Number.isFinite(asset.videoFrameTime) ? asset.videoFrameTime : 0;
    slider.value = Math.max(0, Math.min(t, duration || t)).toFixed(3);
    valueEl.textContent = `${Number(slider.value).toFixed(1)}s`;

    if (!duration && asset.videoKey) {
        ensureCurrentVideoMetadata().then(() => {
            // resync after metadata loads
            syncVideoFrameControlsUI();
        });
    }
}

async function updateVideoFrameForCurrentScreenshot(timeSeconds) {
    const screenshot = getCurrentScreenshot();
    const asset = getCurrentLocalizedAsset();
    if (!screenshot || !asset || asset.type !== 'video') return;
    if (!asset.videoKey) return;

    const blob = await getMediaBlob(asset.videoKey);
    if (!blob) return;

    const frame = await extractVideoFrameFromBlob(blob, timeSeconds);
    const posterSrc = frame.dataUrl;
    ...
    asset.image = posterImg;
    asset.src = posterSrc;
    asset.name = asset.videoName || asset.name;
    asset.videoFrameTime = Number.isFinite(frame.usedTime) ? frame.usedTime : timeSeconds;
    asset.videoDuration = frame.duration;
    asset.videoWidth = frame.width;
    asset.videoHeight = frame.height;

    screenshot.image = posterImg; // legacy compat
...
async function exportCurrent() {
    if (state.screenshots.length === 0) {
        await showAppAlert('Please upload media first', 'info');
        return;
    }
    ...
    // Image export (or 3D) as PNG
    const link = document.createElement('a');
    link.download = `media-${state.selectedIndex + 1}.png`;
    link.href = renderCurrentStateToPngDataUrl({ transparentOnlyFrame: isExportTransparentEnabled() });
    link.click();
}

function getVideoExportProfile({ preferAlpha = false } = {}) {
    if (typeof MediaRecorder === 'undefined') return null;
    const mp4Candidates = [
        { mimeType: 'video/mp4;codecs="avc1.42E01E,mp4a.40.2"', ext: 'mp4' },
        { mimeType: 'video/mp4', ext: 'mp4' }
    ];
    // Alpha-friendly candidates (WebM often preserves alpha from canvas)
    const alphaCandidates = [
        { mimeType: 'video/webm;codecs=vp9', ext: 'webm' },
        { mimeType: 'video/webm;codecs=vp8', ext: 'webm' },
        { mimeType: 'video/webm', ext: 'webm' }
    ];
    const candidates = preferAlpha ? [...alphaCandidates, ...mp4Candidates] : [...mp4Candidates, ...alphaCandidates];
...
async function exportCurrentVideo() {
    const asset = getCurrentLocalizedAsset();
    if (!asset?.videoKey) {
        await showAppAlert('Missing video data for this media item', 'info');
        return;
    }
    if (typeof MediaRecorder === 'undefined') {
        await showAppAlert('Video export is not supported in this browser', 'info');
        return;
    }

    const wantTransparent = isExportTransparentEnabled();
    const profile = getVideoExportProfile({ preferAlpha: wantTransparent });
    if (!profile) {
        await showAppAlert('Video export is not supported in this browser', 'info');
        return;
    }
    if (profile.ext !== 'mp4' && !wantTransparent) {
        // Best-effort: use MP4 when available, otherwise fall back safely
        await showAppAlert('MP4 export is not supported in this browser. Exporting WEBM instead.', 'info');
    }
    if (profile.ext === 'mp4' && wantTransparent) {
        // MP4 generally won't preserve alpha
        await showAppAlert('Transparent video export is not supported in MP4 in this browser. Exporting WEBM instead (if supported).', 'info');
        const alphaProfile = getVideoExportProfile({ preferAlpha: true });
        if (alphaProfile?.ext === 'webm') {
            // Switch to alpha-friendly profile
            // eslint-disable-next-line no-unused-vars
            profile.mimeType = alphaProfile.mimeType;
            // eslint-disable-next-line no-unused-vars
            profile.ext = alphaProfile.ext;
        }
    }
    ...
    const blob = await getMediaBlob(asset.videoKey);
    if (!blob) {
        await showAppAlert('Could not load video from storage', 'info');
        return;
    }

    const ss = getScreenshotSettings();
    const use3D = ss.use3D || false;
    const can3D = !!use3D && typeof renderThreeJSToCanvas === 'function' && typeof updateScreenTexture === 'function' && !!phoneModelLoaded;

    // Create a video element for export (2D pipeline)
    const url = URL.createObjectURL(blob);
    const video = document.createElement('video');
    video.muted = true;
    video.playsInline = true;
    ...
    // Decide duration to export
    const startT = Number.isFinite(asset.videoFrameTime) ? asset.videoFrameTime : 0;
    const duration = Number.isFinite(asset.videoDuration) && asset.videoDuration > 0 ? asset.videoDuration : null;
    const MAX_EXPORT_VIDEO_SECONDS = 30;
    const endT = duration ? Math.min(duration, startT + MAX_EXPORT_VIDEO_SECONDS) : (startT + 5);
    const targetSeconds = Math.max(0.1, Math.min(MAX_EXPORT_VIDEO_SECONDS, (duration ? Math.max(0.1, duration - startT) : 5)));

    // Recorder setup
    const fps = 30;
    const stream = exportCanvas.captureStream(fps);
    const chunks = [];
    const recorder = new MediaRecorder(stream, profile.mimeType ? { mimeType: profile.mimeType } : undefined);
    recorder.ondataavailable = (e) => {
        if (e.data && e.data.size > 0) chunks.push(e.data);
    };

    showExportProgress('Exporting media...', 'Rendering frames', 0);

    try {
        recorder.start(250);

        if (can3D) {
            // 3D: let three-renderer.js drive the video texture, and just composite to exportCanvas
            if (typeof setScreenVideoPlaybackEnabled === 'function') {
                try { setScreenVideoPlaybackEnabled(true); } catch (e) {}
            }
            if (typeof setScreenVideoTime === 'function') {
                try { setScreenVideoTime(startT); } catch (e) {}
            }
            updateScreenTexture(); // initialize video texture
            ...
            await new Promise((resolve) => {
                const tick = () => {
                    ...
                    const dims2 = { width: exportCanvas.width, height: exportCanvas.height };
                    exportCtx.clearRect(0, 0, dims2.width, dims2.height);
                    if (!wantTransparent) {
                        drawBackgroundToContext(exportCtx, dims2, bg);
                        if (bg.noise) {
                            drawNoiseToContext(exportCtx, dims2, bg.noiseIntensity);
                        }
                    }
                    renderThreeJSToCanvas(exportCanvas, dims2.width, dims2.height);
                    if (!wantTransparent) {
                        drawTextToContext(exportCtx, dims2, txt);
                    }

                    const pct = Math.max(0, Math.min(90, Math.round((elapsed / Math.max(0.01, targetSeconds)) * 90)));
                    if (pct !== lastProgress) {
                        lastProgress = pct;
                        showExportProgress('Exporting media...', `${pct}%`, pct);
                    }
            ...
            // 2D pipeline (rounded rect + frame, etc.)
            await waitForEvent(video, 'loadedmetadata', 20000);
            try {
                video.currentTime = Math.max(0, startT);
                await waitForEvent(video, 'seeked', 20000);
            } catch (e) {
                // ignore
            }
            try { await video.play(); } catch (e) { /* still try to render */ }
            ...
            await new Promise((resolve) => {
                const tick = () => {
                    ...
                    drawScreenshotToContext(exportCtx, dims2, video, ss);
                    if (!wantTransparent) {
                        drawTextToContext(exportCtx, dims2, txt);
                    }

                    const t = video.currentTime || 0;
                    const pct = Math.max(0, Math.min(90, Math.round(((t - startT) / Math.max(0.01, (endT - startT))) * 90)));
                    if (pct !== lastProgress) {
                        lastProgress = pct;
                        showExportProgress('Exporting media...', `${pct}%`, pct);
                    }

                    if ((video.currentTime || 0) >= endT || (performance.now() - startedAt) > (MAX_EXPORT_VIDEO_SECONDS + 5) * 1000) {
                        resolve();
                        return;
                    }
                    requestAnimationFrame(tick);
        ...
        showExportProgress('Finalizing...', '', 95);
        const out = new Blob(chunks, { type: recorder.mimeType || (profile.ext === 'mp4' ? 'video/mp4' : 'video/webm') });

        const link = document.createElement('a');
        link.download = `video-${state.selectedIndex + 1}.${profile.ext}`;
        ...
        hideExportProgress();
        await showAppAlert('Video export failed in this browser', 'info');
    } finally {
        URL.revokeObjectURL(url);
...
// Render the *current* selected media (current language) as a video blob.
// Used by exportAll() so we can mix PNG + MP4/WEBM in a single ZIP.
async function renderCurrentStateToVideoBlob() {
    const asset = getCurrentLocalizedAsset();
    const ss = getScreenshotSettings();
    const use3D = ss.use3D || false;
    if (!asset?.videoKey || asset.type !== 'video') return null;
    if (typeof MediaRecorder === 'undefined') return null;
    ...
    // 3D: drive Three.js phone render (including video-in-screen)
    if (use3D && typeof renderThreeJSToCanvas === 'function' && typeof updateScreenTexture === 'function') {
        try {
            if (typeof showThreeJS === 'function') {
                showThreeJS(true);
            }
            if (typeof switchPhoneModel === 'function') {
                switchPhoneModel(ss.device3D || 'iphone');
            }

            // Wait briefly for model load if needed
            if (!phoneModelLoaded) {
                await waitForCondition(() => !!phoneModelLoaded, 20000, 50);
            }

            // Ensure Three.js has the correct video wired for current selection
            await Promise.resolve(updateScreenTexture());

            const startT = Number.isFinite(asset.videoFrameTime) ? asset.videoFrameTime : 0;
            const duration = Number.isFinite(asset.videoDuration) && asset.videoDuration > 0 ? asset.videoDuration : null;
            const MAX_EXPORT_VIDEO_SECONDS = 30;
            const targetSeconds = duration ? Math.min(MAX_EXPORT_VIDEO_SECONDS, Math.max(0.1, duration - startT)) : 5;
            ...
            await new Promise((resolve) => {
                ...
                const tick = () => {
                    ...
                    // Clear and optionally draw background
                    exportCtx.clearRect(0, 0, dims.width, dims.height);
                    if (!wantTransparent) {
                        const bg = getBackground();
                        drawBackgroundToContext(exportCtx, dims, bg);
                    ...
                    // Force the 3D screen video time (if available) then render
                    if (typeof setScreenVideoTime === 'function') {
                        try { setScreenVideoTime(t); } catch (e) {}
                    }
                    renderThreeJSToCanvas(exportCanvas, dims.width, dims.height);

                    if (!wantTransparent) {
                        drawTextToContext(exportCtx, dims, getText());
                    }
            ...
            return {
                blob: new Blob(chunks, { type: recorder.mimeType || (profile.ext === 'mp4' ? 'video/mp4' : 'video/webm') }),
                ext: profile.ext
            };
        } catch (e) {
            console.error('renderCurrentStateToVideoBlob (3D) failed:', e);
            return null;
        }
    }
    ...
    try {
        ...
        return { blob: new Blob(chunks, { type: recorder.mimeType || (profile.ext === 'mp4' ? 'video/mp4' : 'video/webm') }), ext: profile.ext };
    } catch (e) {
        console.error('renderCurrentStateToVideoBlob failed:', e);
        return null;
    } finally {
        URL.revokeObjectURL(url);
        try { video.removeAttribute('src'); video.load(); } catch (e) {}
...
async function exportAllForLanguage(lang) {
    ...
    showExportProgress('Exporting...', `Preparing ${langName} media`, 0);
    ...
    for (let i = 0; i < state.screenshots.length; i++) {
        ...
        showExportProgress('Exporting...', `Media ${i + 1} of ${total}`, percent);
        ...
        // If video media, export as MP4 (preferred) into the zip
        if (asset?.type === 'video' && asset.videoKey && typeof MediaRecorder !== 'undefined') {
            showExportProgress('Exporting...', `Media ${i + 1} of ${total}`, percent);
            const videoOut = await renderCurrentStateToVideoBlob();
            if (videoOut?.blob) {
                zip.file(`video-${i + 1}.${videoOut.ext || 'mp4'}`, videoOut.blob);
                continue;
            }
            // fallback: if video export failed, fall through to PNG
        }

        // PNG export (respects transparent export setting)
        const dataUrl = renderCurrentStateToPngDataUrl({ transparentOnlyFrame: isExportTransparentEnabled() });
        const base64Data = dataUrl.replace(/^data:image\/png;base64,/, '');
        zip.file(`media-${i + 1}.png`, base64Data, { base64: true });
    }
    ...
    const link = document.createElement('a');
    link.download = `media_${state.outputDevice}_${lang}.zip`;
...
async function exportAllLanguages() {
    ...
    for (let langIdx = 0; langIdx < state.projectLanguages.length; langIdx++) {
        ...
        for (let i = 0; i < state.screenshots.length; i++) {
            ...
            showExportProgress('Exporting...', `${langName}: Media ${i + 1} of ${totalScreenshots}`, percent);
            ...
            if (asset?.type === 'video' && asset.videoKey && typeof MediaRecorder !== 'undefined') {
                const videoOut = await renderCurrentStateToVideoBlob();
                if (videoOut?.blob) {
                    zip.file(`${lang}/video-${i + 1}.${videoOut.ext || 'mp4'}`, videoOut.blob);
                    continue;
                }
            }
            ...
            const dataUrl = renderCurrentStateToPngDataUrl({ transparentOnlyFrame: isExportTransparentEnabled() });
            const base64Data = dataUrl.replace(/^data:image\/png;base64,/, '');
            zip.file(`${lang}/media-${i + 1}.png`, base64Data, { base64: true });
        }
    }
    ...
    const link = document.createElement('a');
    link.download = `media_${state.outputDevice}_all-languages.zip`;
    link.href = URL.createObjectURL(content);
    link.click();
    URL.revokeObjectURL(link.href);
}
