...
// Video playback for dynamic screen textures (videos on the phone screen)
let screenVideoKey = null;
let screenVideoEl = null;
let screenVideoUrl = null;
let screenVideoCanvas = null;
let screenVideoCtx = null;
let screenVideoRafId = null;
let screenVideoPlaybackEnabled = true;
let screenVideoInitKey = null;
let screenVideoInitPromise = null;
let screenVideoInitToken = 0;
...
function createRoundedScreenImage(image, cornerRadius) {
    ...
    const { w: srcW, h: srcH } = getSourceDimensions(image);
    canvas.width = srcW;
    canvas.height = srcH;
...
function stopScreenVideoLoop() {
    if (screenVideoRafId) {
        cancelAnimationFrame(screenVideoRafId);
        screenVideoRafId = null;
    }
    // Invalidate any in-flight init
    screenVideoInitToken++;
    if (screenVideoEl) {
        try { screenVideoEl.pause(); } catch (e) {}
        try { screenVideoEl.removeAttribute('src'); screenVideoEl.load(); } catch (e) {}
    }
    if (screenVideoUrl) {
        try { URL.revokeObjectURL(screenVideoUrl); } catch (e) {}
    }
    screenVideoKey = null;
    screenVideoEl = null;
    screenVideoUrl = null;
    screenVideoCanvas = null;
    screenVideoCtx = null;
}

function drawRoundedToCanvas(ctx, w, h, r, drawFn) {
...
function startScreenVideoTickLoop() {
    if (screenVideoRafId) return;
    if (!screenVideoPlaybackEnabled) return;
    if (!screenVideoEl || !screenVideoCanvas || !screenVideoCtx || !screenTexture || !screenVideoKey) return;

    const tick = () => {
        const ss = typeof getScreenshotSettings === 'function' ? getScreenshotSettings() : null;
        const currentLang = (typeof state !== 'undefined' && state.currentLanguage) ? state.currentLanguage : 'en';
        const currentAsset = state?.screenshots?.[state.selectedIndex]?.localizedImages?.[currentLang] || null;

        if (!ss?.use3D || !currentAsset || currentAsset.type !== 'video' || currentAsset.videoKey !== screenVideoKey || !screenVideoEl || !screenVideoCtx || !screenVideoCanvas || !screenTexture) {
            stopScreenVideoLoop();
            return;
        }

        if (!screenVideoPlaybackEnabled) {
            try { screenVideoEl.pause(); } catch (e) {}
            screenVideoRafId = null;
        ...
        const config = deviceConfigs[currentDeviceModel] || deviceConfigs.iphone;
        const cornerRadius = Math.round((screenVideoCanvas.width || 0) * config.cornerRadiusFactor);

        const { w, h } = getSourceDimensions(screenVideoEl);
        if (w && h) {
            drawRoundedToCanvas(screenVideoCtx, screenVideoCanvas.width, screenVideoCanvas.height, cornerRadius, () => {
                screenVideoCtx.drawImage(screenVideoEl, 0, 0, screenVideoCanvas.width, screenVideoCanvas.height);
            });
            screenTexture.needsUpdate = true;
        }

        // Render can be driven by renderThreeJSToCanvas, but keep this for interactive preview too
        requestThreeJSRender();

        screenVideoRafId = requestAnimationFrame(tick);
    };
...
async function updateScreenTexture() {
    ...
    // Try to detect localized asset (to support videos)
    const lang = state.currentLanguage || 'en';
    const asset = screenshot.localizedImages?.[lang] || null;

    // Video: animate video frames on the screen
    if (asset?.type === 'video' && asset.videoKey && typeof getMediaBlob === 'function') {
        try {
            const config = deviceConfigs[currentDeviceModel] || deviceConfigs.iphone;

            // If we're already initialized for this video key, just ensure the loop is running.
            if (screenVideoEl && screenVideoKey === asset.videoKey && screenVideoCanvas && screenVideoCtx && screenTexture && customScreenPlane) {
                if (screenVideoPlaybackEnabled) {
                    try { await screenVideoEl.play(); } catch (e) {}
                } else {
                    try { screenVideoEl.pause(); } catch (e) {}
                }
                startScreenVideoTickLoop();

                return;
            }

            // If a different video is active, reset
            if (screenVideoKey && screenVideoKey !== asset.videoKey) {
                stopScreenVideoLoop();
            }

            // Avoid re-entrant async initialization loops
            if (screenVideoInitPromise && screenVideoInitKey === asset.videoKey) {
                return;
            }

            // Reset any prior video state before initializing
            stopScreenVideoLoop();
            screenVideoKey = asset.videoKey;

            const initToken = screenVideoInitToken;
            screenVideoInitKey = asset.videoKey;
            screenVideoInitPromise = (async () => {
                if (initToken !== screenVideoInitToken) return;

                const blob = await getMediaBlob(asset.videoKey);
                if (!blob) throw new Error('Missing video blob');
                if (initToken !== screenVideoInitToken) return;
                ...
                screenVideoEl.loop = true;
                screenVideoEl.preload = 'auto';
                screenVideoEl.src = screenVideoUrl;

                // Wait for metadata (with timeout)
                await new Promise((resolve, reject) => {
                    const onMeta = () => { cleanup(); resolve(); };
                    const onTimeout = () => { cleanup(); reject(new Error('Video metadata timeout')); };
                    const cleanup = () => {
                        screenVideoEl.removeEventListener('loadedmetadata', onMeta);
                        if (timer) clearTimeout(timer);
                    };
                    const timer = setTimeout(onTimeout, 20000);
                    screenVideoEl.addEventListener('loadedmetadata', onMeta, { once: true });
                });
                if (initToken !== screenVideoInitToken) return;
                ...
                // Seek to chosen start frame (if set)
                const startT = Number.isFinite(asset.videoFrameTime) ? asset.videoFrameTime : 0;
                try {
                    screenVideoEl.currentTime = Math.max(0, startT);
                } catch (e) {}

                screenVideoCanvas = document.createElement('canvas');
                screenVideoCanvas.width = w;
                screenVideoCanvas.height = h;
                screenVideoCtx = screenVideoCanvas.getContext('2d');

                // Replace texture & material once (canvas updates each frame)
                if (screenTexture) {
                    screenTexture.dispose();
                }
                screenTexture = new THREE.Texture(screenVideoCanvas);
                screenTexture.needsUpdate = true;
                ...
                if (customScreenPlane) {
                    // Dispose old material safely (could be array in some models)
                    if (Array.isArray(customScreenPlane.material)) {
                        customScreenPlane.material.forEach(m => m?.dispose?.());
                    } else {
                        customScreenPlane.material?.dispose?.();
                    }
                ...
                // Start playback/loop if enabled
                if (screenVideoPlaybackEnabled) {
                    try { await screenVideoEl.play(); } catch (e) {}
                    startScreenVideoTickLoop();
                } else {
                ...
                console.warn('3D video texture init failed:', e);
                stopScreenVideoLoop();
            }).finally(() => {
                screenVideoInitKey = null;
                screenVideoInitPromise = null;
            });

            return;
        } catch (e) {
            // If anything goes wrong with video path, fall back to static image texture below
            stopScreenVideoLoop();
        }
    }

    // Non-video: stop any video loop and use static image
    stopScreenVideoLoop();
    ...
    const config = deviceConfigs[currentDeviceModel] || deviceConfigs.iphone;
    const { w } = getSourceDimensions(screenshotImage);
    const cornerRadius = Math.round(w * config.cornerRadiusFactor);
...
// Allow app.js to control 3D screen video playback
function setScreenVideoPlaybackEnabled(enabled) {
    screenVideoPlaybackEnabled = !!enabled;
    if (screenVideoEl) {
        if (screenVideoPlaybackEnabled) {
            try { screenVideoEl.play(); } catch (e) {}
...
function setScreenVideoTime(timeSeconds) {
    if (!screenVideoEl) return;
    try {
        screenVideoEl.currentTime = Math.max(0, timeSeconds || 0);
    } catch (e) {}

    // Draw one frame immediately if possible
    if (screenVideoCtx && screenVideoCanvas) {
        const config = deviceConfigs[currentDeviceModel] || deviceConfigs.iphone;
        const cornerRadius = Math.round((screenVideoCanvas.width || 0) * config.cornerRadiusFactor);
        drawRoundedToCanvas(screenVideoCtx, screenVideoCanvas.width, screenVideoCanvas.height, cornerRadius, () => {
            screenVideoCtx.drawImage(screenVideoEl, 0, 0, screenVideoCanvas.width, screenVideoCanvas.height);
